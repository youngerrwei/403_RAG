...
if [[ ${#MCP_INTERNAL_TOKEN} -lt 32 ]]; then
    echo "[错误] MCP_INTERNAL_TOKEN 未配置或强度不足，请先运行 bash scripts/setup_mcp.sh" >&2
    exit 1
fi

CONDA_CMD="$(find_conda || true)"
if [[ -z "$CONDA_CMD" ]]; then
    echo "[错误] 未找到 conda，请先运行 bash scripts/setup_mcp.sh" >&2
    exit 1
fi

MCP_PYTHON="$(conda_env_python "$CONDA_CMD" "$MCP_CONDA_ENV" || true)"
if [[ -z "$MCP_PYTHON" ]]; then
    echo "[错误] MCP 环境不存在或无 Python: $MCP_CONDA_ENV；请先运行 bash scripts/setup_mcp.sh" >&2
    exit 1
fi

cd "$PROJECT_ROOT"
export PYTHONPATH="$PROJECT_ROOT/src${PYTHONPATH:+:$PYTHONPATH}"
exec "$MCP_PYTHON" -m lab_rag.mcp_server
